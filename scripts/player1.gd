...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("ui_accept") and is_on_floor():
		...
		# Verifica se está atacando ao pular
		if is_attacking or is_mega_attacking:
			animated_sprite.play("jump_attack")  # Animação de pulo com ataque
		else:
			animated_sprite.play("jump")  # Animação de pulo normal
		return  # Sai da função para não tocar outras animações
	...
	if not is_attacking and not is_mega_attacking:
		if direction != 0:
			...
			if is_on_floor():
				...
				animated_sprite.flip_h = direction < 0  # Inverte o sprite com base na direção
		else:
